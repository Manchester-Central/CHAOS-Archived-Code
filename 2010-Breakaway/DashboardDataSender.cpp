...
//							dash.AddU8((unsigned char) i * 255 / 10);
		...
		dash.AddCluster();
		{
			dash.AddDouble(DashData->BallUltraSonic_Distance_DashOut);
		}
		dash.FinalizeCluster();
		dash.AddCluster();
		{
			dash.AddDouble(DashData->CameraErrorAngle_Dashout);
			dash.AddDouble(DashData->GyroAngle_DashOut);
			dash.AddDouble(DashData->SetPoint_DashOut);
	        dash.AddDouble(DashData->KickerExtendEistance_DashOut); //normwise to cm (0-24)
			dash.AddDouble(DashData->KickerPotDistance_DashOut); //Linear Pot Distance in CM
			dash.AddDouble(DashData->KLPot_Value_DashOut);
			dash.AddDouble(DashData->ALPot_Value_DashOut);
			dash.AddDouble(DashData->WheelStowDistanceGoal);
			dash.AddDouble(DashData->AutoTargetDistance_DashOut);
			dash.AddDouble(DashData->AutoExtendDistance_DashOut);
		}
		dash.FinalizeCluster();
		dash.AddCluster();
		{
			dash.AddBoolean(DashData->Trigger_DashOut);
			dash.AddBoolean(DashData->PressureSwitch_DashOut);
		}
		dash.FinalizeCluster();
		dash.AddCluster();
		{
			dash.AddI16(DashData->KickerCurrentState_DashOut);
			dash.AddI16(DashData->AutonomousCurrentState_DashOut);
		}
		dash.FinalizeCluster();
		dash.AddCluster();
		{
			dash.AddI8(DashData->DribblerHasBall_DashOut);
		}
		dash.FinalizeCluster();
	}
	dash.FinalizeCluster();
