...
DashboardDataFormat::DashboardDataFormat(DashSharing *DashDataPass)
{
...
void DashboardDataFormat::SendIOPortData()
{
	...
	dash.AddCluster();
	{
		...
		dash.AddCluster();
		{
				...
				dash.AddBoolean(DashData->LightIntake);
				dash.AddBoolean(DashData->LightConveyor);
				dash.AddBoolean(DashData->LightOutput);
		}
		dash.FinalizeCluster();
		dash.AddCluster();
		{
			dash.AddFloat(DashData->SensedDistance);
		}
		dash.FinalizeCluster();
