	...
	: mBuffer( 5, 0.0 ), mBufferLeft(15, 0.0), mBufferRight(15, 0.0)
{
	...
	AutomaticBridgeButtonBool = false;
...
//		}
		...
		if(DriverGamepad->GetNumberedButton(TOGGLE_RETURN_NORMAL_BUTTON) == true)
		{
			AutomaticBridgeButtonBool = true;
		}
		if(BridgeToggleUpSwitch->Get() == true && DriverGamepad->GetNumberedButton(TOGGLE_DOWN_BUTTON) != true && AutomaticBridgeButtonBool == true)
		{
		...
		else if(DriverGamepad->GetNumberedButton(TOGGLE_DOWN_MANUAL_BUTTON) == true && BridgeToggleDownSwitch->Get() == true)
		{
			BridgeToggleMotor->Set(-0.5);
			AutomaticBridgeButtonBool = false;
		}
		else
		{
		...
		else if((DistanceSensor->GetVoltage() * 102.4) > MINIMUM_BRIDGE_DIST && (DistanceSensor->GetVoltage() * 102.4) < MAXIMUM_BRIDGE_DIST && OperatorGamepad->GetNumberedButton(BUTTON_BRIDGE_DISTANCE))
		{
			LEDSignal->Set(Relay::kForward);
		}
		else
		...
		else
		{
			...
			if(OperatorGamepad->GetNumberedButton(BUTTON_DIRECT_DRIVE) == false)
			{
			...
			if(OperatorGamepad->GetNumberedButton(BUTTON_REDUCED_SPEED) == true)
			{
				LeftRampedSpeed = (LeftRampedSpeed * REDUCED_SPEED_COEF);
				RightRampedSpeed = (RightRampedSpeed * REDUCED_SPEED_COEF);
			}
		...
		DashShare->SensedDistance = (filteredDistance);
		...
		switch(AutoNextState)
		{
			...
			case AUTO_STATE_SHOOT:
				...
				if(ScoringMechanism->CheckState() == Manipulator::MANIP_STATE_DISABLED && AutoCurrentState == AUTO_STATE_SHOOT)
				{
					if(Autonomous1)
					{
						if(abs(RightDriveEncoder->GetDistance()) > 1)
						{
...
void Main::RobotInit(void)
{
	...
	AxisCamera &camera = AxisCamera::GetInstance("10.1.31.11");
