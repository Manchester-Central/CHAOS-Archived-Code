...
private:
	...
	boost::circular_buffer<float> mBufferLeft;
	boost::circular_buffer<float> mBufferRight;
public:
	DashboardDataFormat *dashboardDataFormat;
	DashSharing *DashShare;
	
	float RightJoystickSpeed;
	float LeftJoystickSpeed;
	float RightRampedSpeed;
	float LeftRampedSpeed;
	...
	bool Autonomous1;
	bool DriveStraight(float FinalDistance, bool ForwardDirection);
	bool TurnAngle(float Angle);
	bool DriveTo(float Distance2);
	DigitalInput *DrivingRampSwitch;
	
	bool AutomaticBridgeButtonBool;
