...
static int TwoColorDemo_debugFlag = 1;
 ...
 */
	 ...
	 */
ChaosVision::ChaosVision(void) : teamcolor_DI(7) {
...
//		sinStart = 0.0; // control where to start the sine wave for pan
		...
		td1.luminance.minValue = 43;
		td1.luminance.maxValue = 255;
		// GREEN
		sprintf(td2.name, "GREEN");
		td2.hue.minValue = 60;
		td2.hue.maxValue = 85;
		td2.saturation.minValue = 93;
		td2.saturation.maxValue = 255;
		td2.luminance.minValue = 59;
		td2.luminance.maxValue = 168;
	...
	void ChaosVision::startteleop(void) {
		 //Video Update
...
//		setServoPositions(horizontalDestination, verticalDestination);
	 ...
	 */
#if 0
	void ChaosVision::setServoPositions(float normalizedHorizontal, float normalizedVertical) {
	 ...
	 */
	void ChaosVision::adjustServoPositions(float normDeltaHorizontal, float normDeltaVertical) {
	 ...
	 */
	void ChaosVision::VisionMain(void) 
	{
		
			
		bool teamcolor = teamcolor_DI.Get();
		
		
		//GetWatchdog().Feed();		// turn watchdog off while debugging	

		// calculate gimbal position based on colors found 
		if (FindTwoColors(td1, td2, (teamcolor)? ABOVE: BELOW, &par1, &par2) ) {
			//PrintReport(&par2);
			foundColor = true;
			// reset pan		
//			panIncrement = 0;
			if (par1.imageTimestamp == savedImageTimestamp) {
				...
				targetSize = fabs((par1.center_mass_y_normalized - par2.center_mass_y_normalized));
